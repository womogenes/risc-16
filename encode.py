...
import re
from reg_names import REG_NAMES, regname2idx
...
def encode(inst_str: str, addr: int, labels: dict):
    ...
    def si(x: str, width: int, signed: bool) -> str:
        ...
        x is allowed to be a label, in which case we compute the offset to current address.
        """
    ...
    if opcode in ri_type:
        return int(f"{opcode[ri_type]}_{r(args[0])}_{si(args[1], 7, 0)}")
